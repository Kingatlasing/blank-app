...
> **Status note (2026-09-21).** VoxelCraft and the Scale Model Designer now run
...
> pick a tool from the sidebar; VoxelCraft is `pages/1_VoxelCraft.py`. The
> designer's own guide is
> [`docs/scale-model-designer-newcomer-guide.md`](./scale-model-designer-newcomer-guide.md).
...
substring matching was resolving "a cat" to Notre-Dame Cathedral. And the bundled
reference library (§4d) is now generated by `tools/ingest_library.py` rather than
curated by hand — though it still ships empty, because the sandbox cannot reach the
sources it ingests from.
...
| `research.py` | 465 | Wikipedia → Openverse photo lookup, Wikidata dimensions, and picking a 3D reconstruction strategy |
...
| `local_library.py` | 153 | bundled reference photos and cached dimensions (see §4) |
| `known_facts.py` | 99 | curated landmark dimensions (see §4) |
...
Subjects resolve to an entry by the same rule as the curated table in §4c — alias
inside subject always, subject inside alias only for a two-word-or-longer subject,
both through `matching.phrase_in_text` — so a prompt finds the same subject whether
its data comes from the bundle or the table.
