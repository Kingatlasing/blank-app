...
# (theta0, phi0, sigma_theta, sigma_phi, amplitude) — phi0=0 is straight
# ahead (+z), positive phi sweeps toward +x. sigma_theta/sigma_phi must be
# comfortably wider than the mesh's own vertex spacing (2*pi/n_segments,
# pi/n_rings): a Gaussian narrower than that plunges a single vertex while
# its neighbors barely move, which (combined with the fixed-diagonal bug
# described above) is what produced the garbled-mesh defect during
# development.
_FEATURES = [
    (1.05, 0.0, 0.10, 0.55, 0.06),           # brow ridge
    (1.18, 0.40, 0.15, 0.24, -0.15),         # left eye socket (indent)
    (1.18, -0.40, 0.15, 0.24, -0.15),        # right eye socket (indent)
    (1.40, 0.0, 0.32, 0.16, 0.28),           # nose bridge + tip
    (1.58, 0.52, 0.18, 0.22, 0.08),          # left cheek
    (1.58, -0.52, 0.18, 0.22, 0.08),         # right cheek
    (1.88, 0.0, 0.11, 0.20, -0.08),          # mouth cavity (indent)
    (2.05, 0.0, 0.12, 0.22, 0.06),           # chin definition
    (1.45, 1.5, 0.18, 0.19, 0.20),           # left ear
    (1.45, -1.5, 0.18, 0.19, 0.20),          # right ear
]
...
def build_head_mesh(
    ...
    def radius(theta: float, phi: float) -> float:
        ...
        for (t0, p0, st, sp, amp) in _FEATURES:
