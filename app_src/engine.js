...
const state = {
  ...
  biking: false,
  hiddenEncounters: {}, // MAPS key -> [{x,y,slug,level}] - wild creatures lurking in specific grass tiles
};
...
(function computeClassicGrass() {
  // TILED.maps (the original 5-map region) ships a real per-tile "grass" bitmap
  // baked in from its source data, but CLASSIC_TILED (the 7-city/8-route second
  // region) never got one, so wild encounters on every classic route silently
  // never fired at all - the check `td.grass && ...` was always false there.
  // There's no leftover build script to regenerate that bitmap from the source
...
function onArrive() {
  ...
  if (map.tiledKey) {
    ...
    if (td.grass && td.grass[state.y * td.w + state.x] && map.encounterTable && map.encounterTable.length) {
      checkHiddenEncounter(state.map);
    }
    return;
  }
  // warp check
...
/* ---------------------------- Hidden wild encounters ----------------------------
   Wild creatures lurk at specific grass tiles rather than popping up from a random
   chance on every step - a tile is either hiding a creature or it isn't, and
   walking onto that exact tile is what starts the battle (matches how tall grass
   works in the source games, rather than an invisible dice roll under any grass). */
function isGrassTile(map, x, y) {
  if (map.tiledKey) {
    const td = tiledSourceFor(map).data;
    if (x < 0 || y < 0 || x >= td.w || y >= td.h) return false;
    return !!(td.grass && td.grass[y * td.w + x]);
  }
  const row = map.tiles[y];
  const ch = row ? row[x] : null;
  const def = ch && TILE_DEF[ch];
  return !!(def && def.encounter);
}
function mapDims(map) {
  if (map.tiledKey) { const td = tiledSourceFor(map).data; return { w: td.w, h: td.h }; }
  return { w: map.w, h: map.h };
}
const HIDDEN_ENCOUNTER_DENSITY = 40; // roughly one hidden creature per 40 grass tiles
const HIDDEN_ENCOUNTER_MAX = 6;
function spawnHiddenAt(map, x, y) {
  const enc = weightedPick(map.encounterTable);
  const level = enc.min + Math.floor(Math.random() * (enc.max - enc.min + 1));
  return { x, y, slug: enc.slug, level };
}
function ensureHiddenEncounters(mapKey) {
  const map = MAPS[mapKey];
  if (!map.encounterTable || !map.encounterTable.length) return [];
  if (!state.hiddenEncounters[mapKey]) {
    const { w, h } = mapDims(map);
    const grassTiles = [];
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
      if (isGrassTile(map, x, y) && !tileSolidAt(map, x, y)) grassTiles.push({ x, y });
    }
    const count = grassTiles.length ? Math.min(HIDDEN_ENCOUNTER_MAX, Math.max(1, Math.ceil(grassTiles.length / HIDDEN_ENCOUNTER_DENSITY))) : 0;
    const list = [];
    for (let i = 0; i < count; i++) {
      const idx = Math.floor(Math.random() * grassTiles.length);
      const spot = grassTiles.splice(idx, 1)[0];
      list.push(spawnHiddenAt(map, spot.x, spot.y));
    }
    state.hiddenEncounters[mapKey] = list;
  }
  return state.hiddenEncounters[mapKey];
}
function respawnHiddenEncounter(mapKey) {
  const map = MAPS[mapKey];
  const { w, h } = mapDims(map);
  const list = state.hiddenEncounters[mapKey] || (state.hiddenEncounters[mapKey] = []);
  let tries = 30;
  while (tries--) {
    const nx = Math.floor(Math.random() * w), ny = Math.floor(Math.random() * h);
    if (isGrassTile(map, nx, ny) && !tileSolidAt(map, nx, ny) &&
        !(nx === state.x && ny === state.y) && !list.some(h => h.x === nx && h.y === ny)) {
      list.push(spawnHiddenAt(map, nx, ny));
      return;
    }
  }
}
function checkHiddenEncounter(mapKey) {
  const list = ensureHiddenEncounters(mapKey);
  const idx = list.findIndex(h => h.x === state.x && h.y === state.y);
  if (idx < 0) return;
  const enc = list.splice(idx, 1)[0];
  respawnHiddenEncounter(mapKey);
  startWildBattle(enc.slug, enc.level, false);
}
...
// Real Tuxemon character sheets (mods/tuxemon/sprites/*.png) are exactly 3
// walk frames x 4 directions each - no dedicated idle/blink frame exists to
// draw, on any of the 208 sheets. Rather than invent one, a standing
// character gets a small procedural bob instead: the same real "stand"
// frame (0), animated in place by a couple of pixels so it doesn't just
// freeze - no new art, just motion of the art that's already there.
// idleSeed varies the bob's phase per-character (screen position works well)
// so a crowd of NPCs isn't all breathing in perfect unison.
function idleBobOffset(idleSeed) {
  if (idleSeed == null) return 0;
  const phase = (idleSeed * 137) % 1000;
  return Math.sin((performance.now() + phase) / 480) * 1.3;
}
function drawChar(spriteKey, facing, frame, dx, dy, scale, idleSeed) {
  const im = IMG.chars[spriteKey];
  if (!im || !im.complete) return;
  const f = charFrame(spriteKey, facing, frame);
  const w = 16 * scale, h = 32 * scale;
  dy -= idleBobOffset(idleSeed);
...
function renderOverworld() {
  ...
  for (const n of (map.npcs || [])) {
    ...
    drawables.push({ y: n.y, draw: () => drawChar(n.sprite, n.facing, 0, dx, dy, 4, n.x * 3 + n.y * 7) });
  }
  const pdx = camX * TILE - cam.x, pdy = camY * TILE - cam.y;
  const playerSprite = state.playerSprite || 'player_boy';
  drawables.push({ y: camY + 0.5, draw: () => drawChar(playerSprite, state.facing, state.moving ? (Math.floor(state.animTimer / 6) % 2 + 1) : 0, pdx, pdy, 4, state.moving ? null : 1) });
...
function updateMovementAnim() {
  if (state.moving) {
    state.moveProgress = (state.moveProgress || 0) + (state.biking ? 0.22 : 0.11);
    state.animTimer++;
...
function startTrainerBattle(trainerDef, trainerSprite) {
  ...
  state.screen = 'battle';
  // openBattleUI() unconditionally hides #dialogueBox as part of its own
  // cleanup (clearing any stale dialogue left over from before this call) -
  // pushDialogue() has to run AFTER that, not before, or its fresh preBattle
  // text gets hidden the instant it's shown, leaving nothing for the player
  // to click and dialogueQueue/state.screen stuck out of sync with the
  // battle that's already running underneath.
  openBattleUI();
...
function renderBattleScene() {
  ...
  if (battle.trainerSprite) {
    drawChar(battle.trainerSprite, 'down', 0, canvas.width * 0.78, canvas.height * 0.14, 4, 2);
  }
  ...
  drawChar(playerSprite, 'up', 0, canvas.width * 0.02, canvas.height * 0.55, 4.3, 3);
  if (pm) {
...
function finishBattle(playerWon) {
  ...
  if (playerWon) {
    if (t) {
      ...
      // valid index before calling finishBattle(true), so currentEnemy()
      // is undefined here and .slug throws. enemyTeam[0] is always right
      // for a wild/fixed encounter (there's only ever one).
...
function useItemInBattle(slug) {
  // Revive targets a fainted party member, not the active mon - the active
  // slot is never fainted by the time the bag can be opened (a fainted mon
  ...
  if (slug === 'cureall') {
    ...
    removeItem(slug, 1);
    appendBattleLog(['Used ' + ITEMS_DB[slug].name + ' on ' + monsterDisplayName(pm) + '. Fully healed!']);
...
function openRevivePicker() {
  const fainted = state.party.map((m, i) => ({ m, i })).filter(x => x.m.currentHP <= 0);
  setBattlePrompt('');
  const menu = document.getElementById('battleMenu');
  menu.innerHTML = `<div class="listmenu">` +
    (fainted.length
      ? fainted.map(x => `<button onclick="reviveMon(${x.i})">${monsterDisplayName(x.m)} (fainted)</button>`).join('')
      : '<div class="hptext" style="padding:8px">No fainted monsters to revive.</div>') +
    `</div><button class="backbtn" onclick="openBagInBattle()">Back</button>`;
}
function reviveMon(i) {
  const m = state.party[i];
  m.currentHP = Math.min(m.derived.hp, 20);
  removeItem('revive', 1);
  appendBattleLog([monsterDisplayName(m) + ' was revived!']);
  setTimeout(() => resolveEnemyOnlyTurn(), 500);
}
...
// A true silhouette (solid-color shape, no interior detail) derived from a
// monster's own real sprite via CSS mask-image - the sprite's own alpha
// channel becomes the outline, so the shape is genuine artwork, just not
// colored in yet. Used for "seen but not caught" everywhere in the Bestiary,
// matching the classic Pokedex convention (seen = grey silhouette, caught =
// full color).
function silhouetteImg(slug, widthPx) {
  const src = `url(data:image/png;base64,${ASSET_B64.monsters[slug]})`;
  return `<div style="width:${widthPx}px;height:${Math.round(widthPx*44/64)}px;margin:0 auto;background:#6b7280;image-rendering:pixelated;
    -webkit-mask-image:${src};-webkit-mask-size:contain;-webkit-mask-repeat:no-repeat;-webkit-mask-position:center;
    mask-image:${src};mask-size:contain;mask-repeat:no-repeat;mask-position:center;"></div>`;
}
...
function renderDexDetail(slug) {
  ...
  const evoChain = (mon.evolutions || []).map(e => {
    ...
    const evoImg = evoStatus === 'caught'
      ? `<img src="data:image/png;base64,${ASSET_B64.monsters[e.to]}" style="image-rendering:pixelated;width:48px">`
      : evoStatus === 'seen'
      ? silhouetteImg(e.to, 48)
