...
function renderOverworld() {
  ...
  let camX = state.px !== undefined ? state.px : state.x;
  let camY = state.py !== undefined ? state.py : state.y;
  const originCol = Math.floor(camX - VIEW_COLS / 2);
  const originRow = Math.floor(camY - VIEW_ROWS / 2);
  const offX = (Math.floor(camX * TILE) - Math.floor(originCol * TILE)) - canvas.width / 2 + TILE / 2;
  const offY = (Math.floor(camY * TILE) - Math.floor(originRow * TILE)) - canvas.height / 2 + TILE / 2;

  for (let ry = -1; ry <= VIEW_ROWS + 1; ry++) {
    for (let rx = -1; rx <= VIEW_COLS + 1; rx++) {
      const tx = originCol + rx, ty = originRow + ry;
      const dx = rx * TILE - offX, dy = ry * TILE - offY;
      let ch = '.';
  ...
  const drawables = [];
  for (const o of (map.objects || [])) {
    const dx = (o.x - originCol) * TILE - offX, dy = (o.y - originRow) * TILE - offY;
