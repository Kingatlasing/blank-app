...
- **Real market values where researched** — 409 of the 790 TCG cards carry a `realValue` sourced from actual web searches (not estimated): all 311 Pokémon cards (100%), and 98 of 479 Yu-Gi-Oh cards — every legendary, epic, and rare Yu-Gi-Oh card, plus a partial pass on uncommons (research is bottlenecked by search-budget limits, not by the cards themselves; the remaining Yu-Gi-Oh commons/uncommons still show a rarity-scaled "Est. value" until priced). Cards with a real price show "Value"; the rest show "Est. value."
