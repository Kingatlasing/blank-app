...
- **Include a real redstone logic-gate and contraption library** — NOT, AND, OR, NAND, NOR, XOR, XNOR, and a signal booster, each a genuinely simulated circuit (not a scripted animation) you can flip the lever(s) on and watch respond, plus a Two-Key AND Door and a Pitfall Trap showing gates and wire driving real mechanisms. Search by name or description ("AND gate", "hidden door trap") right from the structure picker.
...
A guided freeform *generator* for whole categories (describe a fortress with traps, or a maze, and have it auto-built) is a natural next step, but it's a fundamentally different feature from the direct block-by-block Build mode this pass added — that one lets you build anything by hand right now. Repeaters/comparators as real placeable components, and real feedback-based latches/clocks (needs the simulation to track history across interactions, not just recompute a settled state each time) are the natural next extensions to the gate library and propagation simulation above. A mob grinder/spawner-based farm is the natural next farm preset once there's a second mob model to justify it.
