...
- **Include a real redstone logic-gate and contraption library** — NOT, AND, OR, NAND, NOR, and a signal booster, each a genuinely simulated circuit (not a scripted animation) you can flip the lever(s) on and watch respond, plus a Two-Key AND Door and a Pitfall Trap showing gates and wire driving real mechanisms. Search by name or description ("AND gate", "hidden door trap") right from the structure picker.
...
Six presets — **NOT**, **Signal Booster** (a buffer/repeater equivalent), **OR**, **NAND**, **NOR**, and **AND** — each a real, physically-buildable vanilla circuit, not a hardcoded truth table. Every one is built from exactly two genuine mechanics: a torch inverting the block it's mounted on, and redstone dust merging (a dust cell powered by whichever of its neighbors is on, for free OR logic with no torch at all). AND/NAND/NOR are then just torch-inverted compositions of those two (De Morgan's laws — AND = NOT(NAND), NAND = OR(NOT a, NOT b), NOR = NOT(OR(a,b))), exactly like a real redstone tutorial would build them, and every truth table is checked against all input combinations by this repo's own test suite before being wired in. Turn on Interact mode and flip the lever(s) — the torches genuinely relight or go dark, and the output wire responds because the simulation actually recomputed it, not because a script set a color.
